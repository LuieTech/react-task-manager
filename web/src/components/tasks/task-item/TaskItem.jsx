  ...
  return (
    <li className={`list-group-item d-flex justify-content-between align-items-center ${task.completed ? 'list-group-item-secondary' : ''}`}>
      ...
      <div className="task-actions d-flex gap-2">
        <i className='fa fa-times text-danger' role='button' onClick={() => onDeleteTask(task.title)}></i>
        <i className={`fa fa-check ${task.completed ? '' : 'text-success'}`} role='button' onClick={() => onCompleteTask(task.title)}></i>
      </div>
    </li>
  )
}
